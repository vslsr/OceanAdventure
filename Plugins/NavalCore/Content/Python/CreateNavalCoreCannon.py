...
# Art is grouped per weapon under NavalArts and gets reorganised as it grows. These paths are
# only where a fresh import lands: an asset that already exists is found by name, wherever it
# has been moved to, so rearranging the folders never breaks the wiring.
...
def find_asset_by_name(asset_name):
    """Where an asset actually lives right now, or None.

    Resolving by name rather than by folder is what lets the art be regrouped under NavalArts
    without every script that names it having to be edited.
    """
...
def main():
    ...
    if cannonball_mesh is None:
        warn(
            f"No {CANNONBALL_MESH_NAME} anywhere under {PLUGIN_ROOT} and no "
            f"{CANNONBALL_SOURCE_FBX.name} in blender/models. Run "
            "blender/script/python/create_naval_cannonball.py, or MigrateSharedCannon.py if "
            "the asset is still under a GameFeature."
        )
    ...
    if cannon_mesh is None:
        warn(
            f"No {CANNON_MESH_NAME} anywhere under {PLUGIN_ROOT}. The gun will be invisible "
            "until one is assigned by hand in BP_Naval_Cannon; export blender/cannon.blend to "
