...
**已在引擎内验证**（2026-09-16，UE 5.7.4）：`OceanCore.Terrain.Parity` 通过，
连同其余四条共 5 项通过、0 失败、0 未运行、无警告，测试进程退出码 `0`。
这条是整个迁移里最先该做、也最难事后补救的一项——§8.1 说明了为什么。
...
**已在引擎内验证**（UE 5.7.4）：编译通过；`OceanCore.Terrain.Mesh` 与
`OceanCore.Terrain.Outline` 通过——顶面投影面积精确平铺、法线与崖面竖直、接缝包含性、
折边判定都成立。

**仍未验证**：`UOceanTerrainChunkComponent` 这一层。自动化测试验的是 builder 吐出的
**数据与拓扑**，验不到「网格朝向对不对、碰撞建没建成、编辑器里看不看得见」——
`FDynamicMesh3` 的接线、`EnableComplexAsSimpleCollision`、绕序都还只有静态正确性。
这些归 `doc/test/OceanCore-Terrain.md` 的 B 段。
...
**已在引擎内验证**（UE 5.7.4）：`OceanCore.Terrain.Patches` 通过——编辑语义与
「通知集合 == 实际网格变化集合」都成立。

**仍未验证**：`UOceanTerrainSubsystem` 与组件的接线，以及编辑器里「抬高一格能看到
网格与碰撞同步更新」。同样归 B 段。
...
> 交给别人执行时按 [`doc/test/OceanCore-Terrain.md`](../test/OceanCore-Terrain.md) 走——
> 那里有可直接粘贴的命令、逐条判据和报告模板。下面这张表是它的设计依据。
...
| 2026-09-16 | **P1 在 UE 5.7.4 内验证通过**：5 条自动化测试全过、0 失败、0 未运行、退出码 0。含 `Parity` 的 33282 格逐格比对，真相层与参考实现位级一致这一条就此关闭。P2/P3 的数据与拓扑同样通过；剩下未验证的收窄为组件层（朝向、碰撞、编辑器可见性），归测试协议 B 段。 |
