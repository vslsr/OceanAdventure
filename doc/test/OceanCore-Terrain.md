...
> **A 段最近一次结果**：2026-09-16，UE 5.7.4，5 项通过 / 0 失败 / 0 未运行，
> 退出码 `0`，无警告。**B 段尚未执行。**
...
用控制台命令，不用画蓝图。先站到一块平地上，按 `~` 打开控制台：

```
Ocean.Terrain.Here
```

它会打印你脚下的格坐标与该格当前状态（高度层、形状、表面、群系、是否已被编辑）。
**报不出来的话先看它说了什么**——「terrain store not initialised」意思是还没有 chunk
注册上来，那是 B-0 没做完，不是编辑功能坏了。

| # | 命令 | 判据 |
| --- | --- | --- |
| B-2a | `Ocean.Terrain.RaiseHere` | 脚下那一格**立刻升高一层**，网格与碰撞一起变——能站上去，不是浮在原高度 |
| B-2b | 再 `Ocean.Terrain.LowerHere` | 回到原状，且命令回显 `[procedural]` 而不是 `[edited]`——**改回默认值必须清掉覆盖** |
| B-2c | 对一格连续 `RaiseHere` 三次 | 形成三层台阶，每层之间是垂直崖面 |
| B-2d | **跨 chunk 边界**：`Ocean.Terrain.Here` 找到一格局部坐标为 0 的格（X 或 Y 是 32 的整数倍），对它 `Ocean.Terrain.Raise X Y` | **相邻 chunk 同步更新**，接缝处不留旧几何 |
| B-2e | `Ocean.Terrain.Flood X Y` 对一格陆地 | 该格降到真正有水深的层并变成水面 |
| B-2f | `Ocean.Terrain.ResetAll` | 全部编辑清空，地形回到程序化原貌，回显 `remain 0` |

**B-2d 是这一段最有价值的一条**——它验的是「一格编辑要通知几个 chunk」那套判定。
`OceanCore.Terrain.Patches` 已经证明通知集合与实际网格变化集合一致，但那是在
builder 层；B-2d 验的是这套通知真的驱动了组件重建。

全部命令：`Here` / `Info X Y` / `Raise X Y [n]` / `Lower X Y [n]` / `RaiseHere [n]` /
`LowerHere [n]` / `Flatten X Y` / `Flood X Y` / `Reset X Y` / `ResetAll`。
Shipping 构建里编译掉；编辑只作用于本地世界（服务端权威是 P4）。
...
B-2a 抬高即时生效     ：是 / 否
B-2b 降回后显示 procedural：是 / 否
B-2c 三层台阶带崖面   ：是 / 否
B-2d 跨界格邻块同步   ：是 / 否
B-2e Flood 有水深     ：是 / 否
B-2f ResetAll 回到原貌：是 / 否
