...
## 修复必须落到 main，不能只留在工作分支（硬性）

**改完就推 main。** 在工作分支上提交、推送分支，然后就去做下一件事——这不算做完。

尤其是：**请别人重编、重跑、复验之前，那份修复必须已经在他们会检出的分支上。**
别人检出的是 `main`，你的修复躺在 `claude/xxx` 上，等于没修。

动手让别人验之前，用这条确认，别靠记忆：

```bash
git fetch origin main
git merge-base --is-ancestor <你的提交> origin/main && echo 在 main 上 || echo 还没落地
```

**已发生**：绕序修复（`7976796`）推了工作分支没落 `main`。随后请用户重编三次，
每次现象完全一样——因为编的都是旧代码。而「修了却没变化」这个现象极具误导性：
它看起来像「修错了方向」，于是排查转去怀疑水面遮挡、怀疑重建链路，
**全是在一个根本不存在的故障上打转**。真正的原因只是那份代码不在他手上。

唯一的例外是**刻意暂不落地**——比如为了让一次重编只带一个变量，把另一半工作
先留在分支上。这种情况必须**明说**：说清楚 `main` 上现在有什么、没有什么、为什么。
不说就等同于上面那次失误，因为对方没办法知道两者的区别。

相关：下一节的验证协议解决「结果怎么回报」，这一节解决「他验的到底是不是你改的那份代码」。
两条都缺一不可——回报再规范，验错了版本也是白验。
